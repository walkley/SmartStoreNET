...
// Note: PreApplicationStartMethod is not supported in ASP.NET Core.
// Initialize() must be called explicitly from application startup code (e.g., Program.cs).
        ...
        private static Assembly Probe(AssemblyReference ar, PluginDescriptor d, bool shadowCopy)
        {
            ...
            try
            {
                ...
                lock (_lock)
                {
                    ...
                    // Note: BuildManager.AddReferencedAssembly() is not available in ASP.NET Core.
                    // Assembly discovery and loading is handled automatically by the runtime.
                }
