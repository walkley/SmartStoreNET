...
namespace SmartStore.Core.Data.Hooks
{
    public class DefaultDbHookHandler : IDbHookHandler
    {
        ...
        private IEnumerable<IDbSaveHook> GetSaveHookInstancesFor(IHookedEntity entry, HookStage stage, bool importantOnly)
        {
            ...
            if (_hooksRequestCache.TryGetValue(requestKey, out var cachedList))
            {
                hooks = cachedList.ToArray();
            }
            else
            {
                hooks = _saveHooks
        ...
        private void RegisterVoidHook(IDbSaveHook hook, IHookedEntity entry, HookStage stage)
        {
            ...
            var key1 = new RequestHookKey(entry, stage, false);
            var key2 = new RequestHookKey(entry, stage, true);
            if (_hooksRequestCache.TryGetValue(key1, out var list1))
            {
                list1.Remove(hook);
if (list1.Count == 0)
                    _hooksRequestCache.Remove(key1);
            }
            if (_hooksRequestCache.TryGetValue(key2, out var list2))
            {
                list2.Remove(hook);
                if (list2.Count == 0)
                    _hooksRequestCache.Remove(key2);
            }
