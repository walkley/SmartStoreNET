...
public Log4netLogger(log4net.Core.ILogger logger, IHttpContextAccessor httpContextAccessor)
{
...
var httpRequest = isAppInitialized ? _httpContextAccessor.HttpContext?.Request : null;
                    ...
                    if (DataSettings.DatabaseIsInstalled() && isAppInitialized)
                    {
                        ...
                        if (container.TryResolve<IWebHelper>(null, out IWebHelper webHelper))
                        {
                            try
                            {
                                ...
                                props["HttpMethod"] = httpRequest?.Method;
