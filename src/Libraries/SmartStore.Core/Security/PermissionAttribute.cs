...
namespace SmartStore.Core.Security
{
    ...
    public partial class PermissionAttribute : IAuthorizationFilter
    {
        /// <summary>
        ...
        protected virtual void HandleUnauthorizedRequest(AuthorizationFilterContext filterContext)
        {
            ...
            if (request.Headers["X-Requested-With"] == "XMLHttpRequest")
            {
                if (message.HasValue())
                {
                    httpContext.Response.Headers.TryAdd("X-Message-Type", "error");
                    httpContext.Response.Headers.TryAdd("X-Message", message);
                }
