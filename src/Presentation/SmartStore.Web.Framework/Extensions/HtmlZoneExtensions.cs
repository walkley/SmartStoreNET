    ...
    public static class HtmlZoneExtensions
    {
        private class DocumentZone : IDisposable
        {
            ...
            public static IEnumerable<string> GetZoneContent(string zone)
            {
                ...
                var storage = /* Added by CTA: TODO: Replace HttpContext.Current with dependency injection pattern using IHttpContextAccessor. */
HttpContext.Current?.GetItem<Multimap<string, string>>(ItemsKey, forceCreation: false);
                return storage?[zone];
            }
            ...
            public static bool HasUniqueKey(string key)
            {
                ...
                var uniqueKeys = /* Added by CTA: TODO: Replace HttpContext.Current with dependency injection pattern using IHttpContextAccessor. */
HttpContext.Current?.GetItem<HashSet<string>>(UniqueKeysKey, forceCreation: false);
                return uniqueKeys != null && uniqueKeys.Contains(key);
            }

            private static HashSet<string> UniqueKeys => /* Added by CTA: TODO: Replace HttpContext.Current with dependency injection pattern using IHttpContextAccessor. */
HttpContext.Current?.GetItem<HashSet<string>>(UniqueKeysKey, () => new HashSet<string>(StringComparer.OrdinalIgnoreCase), true);
